...
_INV_MULT: dict[Multiplicity, Multiplicity] = {
...
for _side in (Side.LEFT, Side.BOTTOM, Side.BACK):
    for _mult in Multiplicity:
        _opp_side = _OPPOSITE_SIDE[_side]
        _opp_mult = _INV_MULT[_mult]
        HOME_TRANSITIONS[Move(_side, _mult)] = HOME_TRANSITIONS[
            Move(_opp_side, _opp_mult)
        ]
...
def _build_color_transitions() -> None:
    ...
    for f in Color:
        for t in Color:
            ...
            dummy.front_color = f
            dummy.top_color = t
            left_c = side_color(dummy, Side.LEFT)
            right_c = side_color(dummy, Side.RIGHT)
            bottom_c = side_color(dummy, Side.BOTTOM)
            back_c = side_color(dummy, Side.BACK)
    ...
    for side in (Side.LEFT, Side.BOTTOM, Side.BACK):
        for mult in Multiplicity:
            opp_side = _OPPOSITE_SIDE[side]
            opp_mult = _INV_MULT[mult]
            for f in Color:
                for t in Color:
                    if f == t or t == opp_color[f]:
                        continue
                    m = Move(side, mult)
                    opp_m = Move(opp_side, opp_mult)
                    COLOR_TRANSITIONS[(m, f, t)] = COLOR_TRANSITIONS[(opp_m, f, t)]
...
def rotate(move: Move, cube: Cube) -> None:
    ...
    new_f, new_t = COLOR_TRANSITIONS[(move, cube.front_color, cube.top_color)]
